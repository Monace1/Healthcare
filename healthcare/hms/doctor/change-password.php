...
if(isset($_POST['submit'])) {
    $hashed_cpass = md5($_POST['cpass']);
    $email = $_SESSION['dlogin'];

    $sql = "SELECT * FROM doctors WHERE password=? AND docEmail=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ss", $hashed_cpass, $email);

    $stmt->execute();
    $result = $stmt->get_result();

    if($result->num_rows > 0) {
        $hashed_npass = md5($_POST['npass']);
        $update_sql = "UPDATE doctors SET password=?, updationDate=? WHERE docEmail=?";
        $update_stmt = $mysqli->prepare($update_sql);
        $update_stmt->bind_param("sss", $hashed_npass, $currentTime, $email);
